...
void Cadastro_de_Proposta(char cursos[50][60], int indice, int escolhacurso, struct cadastro propostas[50][10][2], int dia, int horasdisponiveis[50][12], int contadordetipos[2][4]); //A função que realiza o procedimento de cadastro de uma proposta.
...
int main(){
    ...
    int cadastrando=0, escolhendocurso=1; //Variáveis utilizadas em laços de repetição while e do-while.
    ...
    Usuario_Digita_Os_Cursos_Da_Feira(numcursos, cursos); //Processo de entrada dos cursos que participarão da Feira
...
void Cadastro_de_Proposta(char cursos[50][60], int indice, int escolhacurso, struct cadastro propostas[50][10][2], int dia, int horasdisponiveis[50][12], int contadordetipos[2][4]){
    int i, prof_ou_alun;
    ...
    for (i=0; i<propostas[escolhacurso-1][indice][dia].quantidade_de_proponentes; i++){
        ...
        if (propostas[escolhacurso-1][indice][dia].temprof==0)
            strcat(propostas[escolhacurso-1][indice][dia].titulo, "(Essa proposta possui apenas estudantes!)"); //Se temprof for igual a zero (ou seja, se não tiver professores) essa mensagem aparece ao lado do nome da proposta, no relatório
}
...
int Pega_Hora(int horasdisponiveis[50][12][2], int escolhacurso, int dia){
    ...
    do{
        printf("Escolha um horario para a apresentacao:");
        for (i=0; i<12; i++){
            printf("\n\t[%i] %ih", i+1, i+8);
        ...
        else if (valorhora<0 || valorhora>12)
    ...
    for (dia=0; dia<=1; dia++){ //Nessa rotina, o programa printa "DIA 23/10" e todas as propostas desse dia abaixo, em seguida, printa "24/10" e todas as propostas desse dia abaixo
        ...
        for (i=0; i<numcursos; i++){
            ...
            for (j=0; j<quantpropostas[dia][i]; j++){
                ...
                fprintf(file, "\n\t\t\tHorário: %i", propostas[i][j][dia].hora);
